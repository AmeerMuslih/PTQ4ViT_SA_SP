...
if __name__ == "__main__":
    X = torch.randint(0, 2**6, (197, 64))
    W = torch.randint(0, 2**6, (64, 197))
    bitFlips_number=500000
    bitflip_per_layer = bitFlips_number/24
    bitflip_per_mul = bitflip_per_layer/384
    out = matmul_FI(X,W,SA_DIM,bitflip_per_mul)
    exp_out = np.matmul(X, W)
    print(torch.allclose(out, exp_out.float()))
